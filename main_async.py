...
from jwt_token.auth.auth_bearer import JWTBearer
from jwt_token.auth.auth_handler import sign_jwt, decode_jwt, JWT_EXPIRE_SECONDS
from jwt_token.model import PostSchema, UserSchema, UserLoginSchema
from sqlalchemy import select
from sqlalchemy.orm import Session
from sqlalchemy.ext.asyncio import AsyncSession
from tool_async.tool_ai_pet_multi_async import AIUserSession 
from tool_async import tool_postgreSQL as user_db
import asyncio
...
async def login_user(
    ...
    # 3️⃣ 建立回應物件 & 寫入 HttpOnly Cookie
    resp = JSONResponse(content={"message": "Login successful"})  # 前端只需要知道成功
    max_age_value = JWT_EXPIRE_SECONDS * 30 if user.remember_me else JWT_EXPIRE_SECONDS
    ...
    if is_onLine:
        resp.set_cookie(
            ...
            secure=True,         # 本地測試用；上線務必改 True 且走 HTTPS
            samesite="none",    # 防 CSRF，可視前後端同源情況調整成 "Lax"
            path="/",  # 非常重要，否則只能在 login 路徑下用
        ...
        value=token,
        httponly=True,        # JS 讀不到 → 防 XSS
        secure=False,         # 本地測試用；上線務必改 True 且走 HTTPS
        samesite="Lax",    # 防 CSRF，可視前後端同源情況調整成 "Lax"
        path="/",  # 非常重要，否則只能在 login 路徑下用
        max_age=max_age_value,    
        )
    print(f"👉 設定 Cookie: access_token (max_age={max_age_value / (60 * 60)} 小時)")
    return resp              # 4️⃣ 最後回傳帶 Cookie 的 Response

@app.get("/sessionJWT", tags=["user"])
async def check_session_jwt(request: Request):
    ...
    return {"msg": "Session is valid", "user_id": payload["user_id"], "user_email": payload["email"]}
